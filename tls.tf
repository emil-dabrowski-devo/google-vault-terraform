...
resource "tls_self_signed_cert" "vault-ca" {
  ...
  provisioner "local-exec" {
    command = "echo '${self.cert_pem}' > ./tls-cert/ca.pem && chmod 0600 ./tls-cert/ca.pem"
  }
}
...
resource "tls_locally_signed_cert" "vault" {
  ...
  provisioner "local-exec" {
    command = "echo '${self.cert_pem}' > ./tls-cert/vault.pem && echo '${tls_self_signed_cert.vault-ca.cert_pem}' >> ./tls-cert/vault.pem && chmod 0600 ./tls-cert/vault.pem"
  }
}
